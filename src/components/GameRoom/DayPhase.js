...
// You might want to pass ROLE_META if it differs, or import it. For now, inline it.
const ROLE_META = {
    mafia: { label: 'Mafia', color: 'text-secondary' },
    doctor: { label: 'Docteur', color: 'text-emerald-400' },
    detective: { label: 'Détective', color: 'text-blue-400' },
    villager: { label: 'Villageois', color: 'text-on-surface' },
}
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="relative z-10 max-w-xl w-full flex flex-col gap-6">
                ...
                {nightResult && (
                    ...
                    >
                        ...
                        <p className="text-on-surface-variant font-bold text-[10px] uppercase tracking-[0.3em] mb-4 text-center relative z-10">📰 Rapport de la nuit</p>
                        {nightResult.payload.eliminated ? (
                            <div className="text-center relative z-10">
                                <p className="text-on-surface text-lg font-medium leading-relaxed">
                                    <span className="text-secondary font-bold text-xl px-2">{nightResult.payload.eliminated.username}</span> a été éliminé·e cette nuit.
                                </p>
                        ...
                        {playerRole === 'detective' && detectiveOwnResult !== null && detectiveOwnResult !== undefined && (
                            ...
                            >
                                <p className="text-[10px] text-on-surface-variant font-bold uppercase tracking-widest mb-2">Résultat de votre enquête</p>
                                <p className={`text-xl font-black flex items-center justify-center gap-2 ${detectiveOwnResult ? 'text-secondary' : 'text-emerald-400'} drop-shadow-md`}>
                                    {detectiveOwnResult ? <><ShieldAlert className="w-6 h-6" /> C'est un membre de la Mafia !</> : <><CheckCircle2 className="w-6 h-6" /> Ce n'est pas la Mafia.</>}
                                </p>
                            </motion.div>
                        )}
                    </motion.div>
                ...
                <div className="glass-panel p-6 sm:p-8 rounded-3xl border border-outline-variant/10 shadow-2xl relative overflow-hidden flex flex-col items-center w-full">
                    ...
                    {isRevote && (
                        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="mb-8 p-4 bg-secondary-container/30 rounded-xl flex flex-col items-center text-center relative z-10 shadow-sm">
                            ...
                            <h3 className="text-secondary font-bold uppercase tracking-widest mt-2">Revote — Égalité !</h3>
                            <p className="text-on-surface-variant text-sm mt-1">Vous ne pouvez voter que pour les joueurs a égalité.</p>
                        </motion.div>
                    )}
                    ...
                    <p className="text-center text-on-surface-variant font-medium text-sm mb-8 sm:mb-10 relative z-10">Votez pour éliminer un suspect.</p>

                    {voted ? (
                        <div className="text-center py-6 sm:py-10 relative z-10 bg-surface-container-highest/50 rounded-2xl w-full">
                            <p className="text-on-surface-variant font-medium tracking-wide mb-6 text-sm sm:text-base">Vote enregistré. Le village délibère...</p>
                                    ...
                                    return (
                                        ...
                                        >
                                            <div className="flex w-full items-center gap-3 sm:gap-4 relative z-10">
                                                ...
                                                {p.user_id === currentUserId && <span className="ml-auto text-[10px] uppercase font-bold text-on-surface-variant tracking-widest bg-surface/50 px-2 py-1 rounded-full shrink-0 hidden sm:inline-block">Vous</span>}
                                            </div>
